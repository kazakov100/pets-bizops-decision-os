...
def test_simulate_target_within_average_pace_for_lower_better():
    ...
    assert result.feasibility_label in {
        "within historical average pace",
        "requires sustaining the best historical quarter, every quarter",
    }
...
def test_simulate_target_exceeds_history_for_unrealistic_lower_better_target():
    ...
    assert result.feasibility_label == "exceeds any historical quarterly move -- not realistic on current trend"
...
def test_simulate_target_higher_better_direction_signs_correctly():
    ...
    assert result.feasibility_label == "within historical average pace"
...
def test_simulate_target_trivial_when_already_past_target():
    ...
    assert result.feasibility_label == "trivial-or-wrong-direction"
